...
apply:
```bash
git merge-base master release-5.4.2-hive.1
```
Note that `oldparent` won't be applied – it's the last common commit in
branch `master` and yours. You can do this visually with gitk as well,
...
You can also use script `./hive/scripts/patch-hive-stuff.sh` to get
required git command:
```bash
TAG="5.4.3" BRANCH="release-5.4.2-hive.1" ./hive/scripts/patch-hive-stuff.sh
```
